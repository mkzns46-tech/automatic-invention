...
function printQuotePdf(quote) {
  ...
  const rows = (quote.lines || []).map((line, index) => `
    <tr>
      ...
      <td>${quoteEscape(line.barcode)}</td>
      <td class="num">${Number(line.qty || 0)}</td>
      <td>${quoteEscape(line.unit || "")}</td>
      <td class="num">${quoteYen(line.unitPrice)}</td>
      <td class="num">${quoteYen(line.amount)}</td>
    </tr>
  `).join("");
...
body { font-family: "Yu Gothic", "Meiryo", sans-serif; color:#10251d; }
...
.company { text-align:right; line-height:1.7; }
...
.summary { width:300px; margin-left:auto; margin-top:18px; }
...
<div class="top">
  ...
  <div class="company">
    <strong>ARICO ARCHERY</strong><br>
    登録番号：<br>
    住所：<br>
    TEL：<br>
...
<div>見積番号：${quoteEscape(quote.quoteNo)}　見積日：${quoteEscape(quote.quoteDate)}　有効期限：${quoteEscape(quote.validUntil)}</div>
<p>下記の通り、お見積りいたします。</p>
<div class="total">合計金額 ${quoteYen(totals.total)}（税込）</div>
<table>
  <thead><tr><th>No</th><th>商品名</th><th>商品コード</th><th>数量</th><th>単位</th><th>税込単価</th><th>税込金額</th></tr></thead>
  <tbody>${rows}</tbody>
</table>
<div class="summary">
  <div><span>小計</span><strong>${quoteYen(totals.subtotal)}</strong></div>
  <div><span>値引き</span><strong>${quoteYen(totals.discount)}</strong></div>
  <div><span>合計</span><strong>${quoteYen(totals.total)}</strong></div>
  <div><span>内消費税</span><strong>${quoteYen(totals.tax)}</strong></div>
</div>
