function printDeliveryPdf(delivery) {
  ...
  <style>
    ...
    <div class="summary"><div><span>小計</span><strong>${money(delivery.subtotal)}</strong></div><div><span>値引き</span><strong>${money(delivery.discount)}</strong></div><div><span>合計</span><strong>${money(delivery.total)}</strong></div><div><span>内消費税</span><strong>${money(delivery.tax)}</strong></div></div>
  </body></html>`);
  win.document.close();
  win.focus();
  win.print();
