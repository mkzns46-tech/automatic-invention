(function(){
  ...
  function buildSmaregiCausePrompt(payload){
    ...
    return [
      ...
      "",
      "確認観点:",
      "- スマレジのオンライン注文では、受注時に「取置き -N」、発送準備時にARICO「出荷 N」、オンライン出荷時にスマレジ「取置き +N」「売上 -N」が発生する。",
      "- 出荷時の「取置き +N」「売上 -N」はセットで純在庫変動0として扱い、受注時の取置きからARICO出荷、スマレジ売上までを一連のオンライン注文として分析すること。",
      "- 数量だけで確定せず、時系列・同時刻処理・ARICO出荷日時・前回チェック締め日時を総合すること。",
