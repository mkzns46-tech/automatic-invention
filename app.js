...
let products=[],logs=[],checks=[],selectedBarcode="",selectedHistoryMode="all",videoStream=null,detector=null,scanning=false,lastScan="",lastScanAt=0;const el=id=>document.getElementById(id);
...
async function reloadAll(){try{showMessage("接続中...");products=await sb("products?select=*&order=name.asc");logs=await sb("inventory_logs?select=*&order=created_at.desc&limit=2000");try{checks=await sb("inventory_checks?select=*&order=checked_at.desc&limit=2000")}catch{checks=[]}render();showMessage("準備OK。スマホカメラまたは手入力で登録できます。","ok")}catch(e){showMessage("データ取得エラー。\n"+e.message,"err")}}
function calcStock(){const m=new Map();for(const p of products)m.set(p.barcode,{barcode:p.barcode,name:p.name,location:p.location||"",base_stock:Number(p.base_stock||0),inQty:0,outQty:0,adjustQty:"",stock:Number(p.base_stock||0)});const ordered=[...logs].sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));for(const l of ordered){const i=m.get(l.barcode);if(!i)continue;const q=Number(l.quantity||0);if(l.type==="入荷"){i.inQty+=q;i.stock+=q}if(l.type==="出荷"){i.outQty+=q;i.stock-=q}if(l.type==="在庫修正"){i.adjustQty=q;i.stock=q}}return[...m.values()]}
...
function render(){renderProductCount();renderStockTable();renderGlobalHistory();renderSelectedProductHistory();renderScanPreview()}function renderProductCount(){const b=el("productCountBadge");if(b)b.textContent=`登録数：${products.length}件`}
...
function renderStockTable(){const q=el("searchInput")?.value?.trim()?.toLowerCase()||"",rows=calcStock().filter(r=>!q||r.barcode.toLowerCase().includes(q)||r.name.toLowerCase().includes(q)||String(r.location).toLowerCase().includes(q));el("stockBody").innerHTML=rows.map(r=>{const lc=gc(r.barcode).sort((a,b)=>new Date(b.checked_at)-new Date(a.checked_at))[0],lt=lc?`${fmt(lc.checked_at)} / ${esc(lc.checked_by)}`:"未チェック";return`<tr class="clickable ${selectedBarcode===r.barcode?"selected-row":""}" data-barcode="${esc(r.barcode)}"><td><button type="button" class="secondary">履歴</button></td><td>${esc(r.barcode)}</td><td>${esc(r.name)}</td><td>${esc(r.location)}</td><td>${r.base_stock}</td><td>${r.inQty}</td><td>${r.outQty}</td><td>${r.adjustQty}</td><td class="${r.stock<0?"stock-minus":"stock-plus"}">${r.stock}</td><td>${lt}</td></tr>`}).join("");document.querySelectorAll("#stockBody tr[data-barcode]").forEach(tr=>tr.addEventListener("click",()=>{selectedBarcode=tr.dataset.barcode;selectedHistoryMode="all";render()}))}
...
async function registerBarcode(barcode){try{barcode=String(barcode||"").trim();if(!barcode)return;const type=el("type").value,staff=el("staff").value.trim(),qtyRaw=el("qty").value.trim(),qty=Number(qtyRaw),memo=el("memo").value.trim();if(!staff){beep(false);showMessage("担当者を入力してください。","err");el("staff").focus();return}if(!qtyRaw||!Number.isFinite(qty)||qty<=0){beep(false);showMessage("数量を入力してください。","err");el("qty").focus();return}const p=gp(barcode);if(!p){beep(false);showMessage(`未登録バーコード：${barcode}。先に商品登録してください。`,"err");el("productBarcode").value=barcode;return}if(type==="出荷"&&gs(barcode)-qty<0){beep(false);showMessage(`在庫不足：${p.name} / 現在庫 ${gs(barcode)} / 出荷数 ${qty}`,"err");return}await sb("inventory_logs",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({type,staff,barcode,product_name:p.name,quantity:qty,memo})});beep(true);showMessage(type==="在庫修正"?`在庫修正：${p.name} / 実在庫を ${qty} に上書き / 担当者：${staff}`:`${type}登録：${p.name} / 担当者：${staff} / 数量 ${qty}`,"ok");el("barcodeInput").value="";el("qty").value="";renderScanPreview();await reloadAll();el("barcodeInput").focus()}catch(e){beep(false);showMessage("登録エラー。\n"+e.message,"err")}}
