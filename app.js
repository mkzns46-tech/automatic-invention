const CONFIG_KEY = "barcode_inventory_supabase_config_v3";
let products = [];
let logs = [];
...
function getConfig() {
  ...
  let key = el("supabaseKey").value.trim();
  url = url.replace(/\/+$/, "");
  return { url, key };
}
...
async function supabaseRequest(path, options = {}) {
  ...
  if (!url || !key) throw new Error("Supabase URL / API KEY が未設定です。");
...
async function loadConfig() {
  ...
  if (saved.url && saved.key) await reloadAll();
}
