...
function isInvoiceEditable(invoiceOrStatus) {
  const status = typeof invoiceOrStatus === "string" ? invoiceOrStatus : invoiceOrStatus?.status;
  return !status || status === INVOICE_STATUS_DRAFT;
}
...
function statusBadge(status) {
  const value = status || INVOICE_STATUS_DRAFT;
  const type = value === INVOICE_STATUS_CANCELLED ? "danger" : value === INVOICE_STATUS_DRAFT ? "muted" : "ok";
...
function fillInvoiceForm(invoice) {
  ...
  document.getElementById("invoiceStatus").value = invoice.status || INVOICE_STATUS_DRAFT;
...
function updateInvoiceLockState(invoice) {
  ...
  if (issueButton) {
    const shouldShow = Boolean(currentInvoiceId && editable);
...
async function issueInvoice() {
  ...
  fillInvoiceForm(invoice);
  renderInvoiceList();
  showSalesPopup("発行確定", `請求書を発行済みにしました\n${invoice.invoiceNo}`, "ok");
}
