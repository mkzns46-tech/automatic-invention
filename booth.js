...
async function createBoothEvent(){
  ...
  try{
    ...
    // Creating an event must not turn the existing common shelf into this
    // event's take-out.  Only an explicit take-out registration may create
    // booth_event_items for the new event.
    const startInventoryResult={count:0,total:0,skipped:true};
    const startInventoryError="";
    ...
    const startInventoryText=startInventoryError
      ...
      : "\n共通イベント棚は今回イベントの持ち出しとして自動登録していません。必要な商品は持ち出し登録から追加してください。";
...
async function renderBoothGachaListPanel(event){
  ...
  area.innerHTML=`<section class="booth-work-card booth-gacha-card">
    <section class="booth-gacha-pick-panel">
      <div class="booth-list-header"><div><h4>ガチャ持ち出し登録</h4><p class="section-note">商品検索・バーコード・商品コードから商品を追加し、最後にまとめて確定します。</p></div></div>
      <div class="booth-gacha-pick-grid">
        <label>商品検索<input id="boothGachaPickSearch" autocomplete="off" placeholder="商品名・バーコード・商品コード" ${closed?"disabled":""}></label>
        <label>バーコード<input id="boothGachaBarcode" autocomplete="off" inputmode="numeric" placeholder="バーコードを入力" ${closed?"disabled":""}></label>
        <label>数量<input id="boothGachaQty" type="number" min="1" step="1" placeholder="数量" ${closed?"disabled":""}></label>
  ...
  setupBoothGachaPickInput(event);
    ...
    <div class="booth-event-inventory-controls">
      ...
      <label>表示
        <select id="boothDepartureListFilter">
          ...
          <option value="start">今回持ち出しあり</option>
          <option value="additional">今回持ち出しあり</option>
...
async function registerBoothGachaMovement(action,data,options={}){
  ...
  try{
    ...
    clearBoothGachaPreview();
    if(!options.skipRefresh)await refreshBoothEventRelatedViews(data.event.id);
    if(!options.silent){
...
function getBoothGachaPickDraft(event){
  const id=String(event?.id||"");
  if(boothGachaPickDraftEventId!==id){boothGachaPickDraftEventId=id;boothGachaPickDraftItems=new Map();}
  return boothGachaPickDraftItems;
}

function renderBoothGachaPickDraft(event){
...
async function saveBoothGachaPickDraft(event){
  ...
  const staff=String(el("boothGachaStaff")?.value||"").trim();
  const memo=String(el("boothGachaMemo")?.value||"").trim();
  if(!items.length){boothShowError("ガチャ登録エラー","商品を1件以上追加してください。");return;}
  if(!staff){boothShowError("ガチャ登録エラー","担当者を選択してください。","boothGachaStaff");return;}
  if(!validateBoothStaffStore(staff,"店舗確認エラー","boothGachaStaff"))return;
  try{
    const checked=[];
    for(const item of items){
      const latest=await findBoothProductByBarcode(item.barcode);
      if(!latest)throw new Error(`商品が見つかりません：${item.barcode}`);
      if(Number(latest.base_stock||0)<item.quantity)throw new Error(`${latest.name||item.barcode} の通常棚在庫が不足しています（現在庫 ${Number(latest.base_stock||0)} / 登録数 ${item.quantity}）`);
      checked.push({...item,product:latest});
    }
...
function setupBoothGachaPickInput(event){
  ...
  el("boothGachaPickDraftList")?.addEventListener("click",e=>{const remove=e.target.closest("[data-gacha-pick-remove]");if(remove)getBoothGachaPickDraft(event).delete(remove.dataset.gachaPickRemove);const change=e.target.closest("[data-gacha-pick-change]");if(change){const item=getBoothGachaPickDraft(event).get(change.dataset.gachaPickBarcode);if(item)item.quantity=Math.max(1,item.quantity+Number(change.dataset.gachaPickChange));}renderBoothGachaPickDraft(event);});
  el("boothGachaPickDraftList")?.addEventListener("change",e=>{const input=e.target.closest("[data-gacha-pick-qty]");if(!input)return;const item=getBoothGachaPickDraft(event).get(input.dataset.gachaPickQty);if(item&&/^[1-9]\d*$/.test(input.value))item.quantity=Number(input.value);renderBoothGachaPickDraft(event);});
  el("boothGachaStartCameraBtn")?.addEventListener("click",()=>{boothScanTarget="gacha";startBoothCarryOutCamera();});
  el("boothGachaStopCameraBtn")?.addEventListener("click",stopBoothCarryOutCamera);
  el("boothCameraZoomRange")?.addEventListener("input",applyBoothCameraZoom); updateBoothCameraZoomLabel(); renderBoothGachaPickDraft(event);
}
...
async function insertBoothCommonEventMovement(event,item,quantity,staff,memo,movementType,snapshot={}){
  const rows=await sb("event_storage_movements",{
    ...
    body:JSON.stringify([{
      ...
      staff,
      memo:memo||"",
      before_qty:snapshot.beforeQty==null?null:Number(snapshot.beforeQty),
      after_qty:snapshot.afterQty==null?null:Number(snapshot.afterQty)
    }])
  });
  return Array.isArray(rows)&&rows[0]?rows[0]:null;
}
...
async function buildBoothDepartureInventoryData(eventId){
  ...
  // The departure list is event-scoped. Common event shelf rows are not
  // current-event take-outs and must never create rows for a new event.
  const barcodes=[...new Set(eventRows.map(row=>String(row.barcode||"").trim()).filter(Boolean))];
  ...
  eventRows.forEach(item=>{
    ...
    normalByBarcode.set(barcode,{
      ...
      searchText,
      taken:legacyTaken,
      commonShelfQty:calculateBoothItemDifference(item),
...
function renderBoothDepartureNormalSection(rows){
  const body=rows.length?rows.map(row=>{
    ...
    return `<tr data-booth-departure-row data-item-id="${esc(row.id||"")}" data-barcode="${esc(row.barcode||"")}">
      <td>${esc(row.product_name||"-")}</td>
      <td>${esc(row.barcode||"-")}</td>
      <td><input class="booth-history-qty-input" data-booth-departure-qty type="number" min="0" step="1" inputmode="numeric" value="${esc(row.taken??0)}" ${row.editable?"":"disabled"}></td>
  ...
  const cards=rows.map(row=>{
    ...
    return `<article class="booth-history-card booth-departure-card" data-booth-departure-row data-item-id="${esc(row.id||"")}" data-barcode="${esc(row.barcode||"")}">
      ...
      <label>今回持ち出し<input class="booth-history-qty-input" data-booth-departure-qty type="number" min="0" step="1" inputmode="numeric" value="${esc(row.taken??0)}" ${row.editable?"":"disabled"}></label>
      ${action}
    </article>`;
  }).join("");
  return `<section class="booth-split-list-section"><h5>今回イベント持ち出し在庫</h5><p class="section-note">このイベントで明示的に登録した持ち出し数量だけを表示します。共通イベント棚や過去イベントの在庫は自動で引き継ぎません。</p><div class="booth-history-table-wrap booth-scroll-table"><table class="booth-history-table booth-departure-list-table"><thead><tr><th>商品名</th><th>バーコード</th><th>今回持ち出し</th><th>現在イベント在庫</th><th>最終更新</th><th>修正</th></tr></thead><tbody>${body}</tbody></table></div><div class="booth-history-cards booth-scroll-cards">${cards}</div></section>`;
}

function syncBoothDepartureCorrectionRow(row){
  if(!row)return;
  const total=Number(row.querySelector("[data-booth-departure-qty]")?.value||0);
  row.querySelectorAll("[data-booth-departure-total]").forEach(node=>{node.textContent=String(Math.max(0,total));});
}
...
async function saveBoothDepartureCorrection(itemId,button){
  ...
  const quantityInput=row?.querySelector("[data-booth-departure-qty]");
  const newQuantityRaw=String(quantityInput?.value||"").trim();
  if(!/^\d+$/.test(newQuantityRaw)){
  ...
  if(totalDelta>0){
    const baseAfter=baseBefore-totalDelta;
    if(baseAfter<0){
      const confirmed=await confirmBoothNegativeCarryOut(baseBefore,totalDelta,baseAfter);
      if(!confirmed)return;
    }
  }
  if(totalDelta<0&&storageBeforeQty<Math.abs(totalDelta)){
  ...
  try{
    ...
    const negativeNote=totalDelta!==0&&nextBaseStock<0
      ? `\n通常棚在庫は${nextBaseStock}になります。マイナス在庫として記録しました。`
      : "";
    boothShowSuccess("持ち出し数を修正しました",`今回持ち出し：${oldTotal} → ${newTotal}${negativeNote}`);
...
var legacyLoadBoothDepartureInventoryList=async function(eventId){
  const list=el("boothDepartureInventoryList");
  if(!list)return;
  try{
    list.innerHTML='<div class="booth-empty">読み込み中...</div>';
...
var legacyLoadBoothDepartureInventoryListWithFilter=async function(eventId){
  const list=el("boothDepartureInventoryList");
  if(!list)return;
  try{
    list.innerHTML='<div class="booth-empty">読み込み中...</div>';
    ...
    const departureFilterMatches=row=>{
      if(filterKey==="start"||filterKey==="additional")return Number(row.taken||0)>0;
...
exportBoothDepartureInventoryCsv=async function(event){
  try{
    ...
    const rows=[
      ["今回イベント持ち出し在庫"],
      ["商品名","バーコード","今回持ち出し","現在イベント在庫","最終更新"],
      ...data.normalRows.map(row=>[row.product_name||"",row.barcode||"",row.taken??0,row.commonShelfQty??0,row.updated_at||""]),
      [],
      ["ガチャ持ち出し在庫"],
      ["商品名","バーコード","ガチャ持ち出し数","戻り実数","使用数","現在ガチャ在庫"],
      ...data.gachaRows.map(row=>[row.product_name||"",row.barcode||"",row.taken,boothGachaDisplayQty(row.returned),boothGachaDisplayQty(row.used),row.remain])
...
exportBoothDepartureInventoryPdf=async function(event){
  try{
    const data=await buildBoothDepartureInventoryData(event?.id);
    const html=`<h1>持ち出し在庫一覧</h1>
      ${boothPdfTable("今回イベント持ち出し在庫",["商品名","バーコード","今回持ち出し","現在イベント在庫","最終更新"],data.normalRows.map(row=>[row.product_name||"",row.barcode||"",row.taken??0,row.commonShelfQty??0,formatBoothDateTime(row.updated_at)]))}
      ${boothPdfTable("ガチャ持ち出し在庫",["商品名","バーコード","ガチャ持ち出し数","戻り実数","使用数","現在ガチャ在庫"],data.gachaRows.map(row=>[row.product_name||"",row.barcode||"",row.taken,boothGachaDisplayQty(row.returned),boothGachaDisplayQty(row.used),row.remain]))}`;
...
if(!window.__aricoBoothDepartureCorrectionHandlersBound){
  document.addEventListener("input",event=>{
    const input=event.target.closest("[data-booth-departure-qty]");
...
var legacyLoadBoothEventReport=async function(eventId){
  const body=el("boothEventReportBody");
  if(!body)return;
  try{
    body.innerHTML='<div class="booth-empty">読み込み中...</div>';
...
function getBoothNormalCarryOutTotalQty(row){
  ...
  if(takenValue!==null&&takenValue!==undefined&&String(takenValue).trim()!==""){
    const taken= Math.max(0,Number(takenValue)||0);
    if(taken>0||splitTotal===0)return taken;
  }
...
var legacyExportBoothEventReportCsv=async function(event){
  try{
    const data=await buildBoothEventReportData(event.id);
    const diffRows=data.diffRows.filter(row=>calculateBoothItemDifference(row)!==0||!row.taken_registered);
...
var legacyExportBoothEventReportPdf=async function(event){
  try{
    const data=await buildBoothEventReportData(event.id);
    const normalProducts=aggregateBoothSalesByProduct(data.normalSales);
    const gachaProducts=aggregateBoothSalesByProduct(data.gachaSales);
  ...
  root.renderBoothReturnPanel=function(event){
    ...
    el("boothReturnCsvInput")?.addEventListener("change",async inputEvent=>{
      ...
      try{
        ...
        csvRows.forEach(row=>{const identity=String(row.identity||row.productCode||"").trim();const lookupKey=`${row.identityType==="barcode"?"barcode":"product_code"}:${identity}`;const product=products.get(lookupKey);if(!product){errors.push(`${row.line}行目 ${identity||"(空欄)"}：${row.identityType==="barcode"?"バーコード":"商品コード"}に一致する商品がありません`);return;}if(!/^\d+$/.test(row.quantityText)){errors.push(`${row.line}行目 ${identity}：数量不正`);return;}const item=state.rows.get(String(product.barcode||""));if(!item){errors.push(`${row.line}行目 ${identity}：このイベントの持ち出し対象外`);return;}const key=String(product.barcode||"");merged.set(key,(merged.get(key)||0)+Number(row.quantityText));});
...
function renderBoothCloseConfirmPanel(event,summary){
  ...
  const tableRows=normalRows.length?normalRows.map(row=>{
    ...
    return '<tr><td>'+esc(row.product_name||"-")+'</td><td>'+esc(row.barcode||"-")+'</td><td>'+esc(taken)+'</td><td>'+esc(sold)+'</td><td>'+esc(returned)+'</td><td>'+esc(unreturned)+'</td><td>'+esc(destinationLabel)+'</td><td>'+esc(current)+'</td><td><strong>'+esc(plannedAfter)+'</strong></td></tr>';
...
// Ver 2.93.02: return facts are recorded in the report, then event close moves
// counted returns to the normal shelf. Common event stock is store-shared.
async function getBoothEventStorageCurrentQty(storeCode,barcode){
  const rows=await sb(`event_storage_stocks?select=storage_qty&store_code=eq.${encodeURIComponent(storeCode)}&barcode=eq.${encodeURIComponent(String(barcode||"").trim())}&limit=1`);
  return Number(Array.isArray(rows)&&rows[0]?.storage_qty||0);
}
...
(function(root){
  ...
  // Return input is metadata only. Physical movement is applied at event close.
  reflectBoothShelfReturnsOnClose=async function(summary,staff){
    const rows=(summary?.rows||summary?.returnPendingRows||summary?.shelfReturnPendingRows||[])
      .filter(row=>row.item_type==="normal"&&(Number(row.returned_qty||0)>0||Math.max(0,Number(row.taken_qty||0)-Number(row.sold_qty||0)-Number(row.returned_qty||0))>0));
    const applied=[];
    try{
      for(const item of rows){
        ...
        const storeCode=getBoothEventStoreCode({store_code:item.store_code});
        const beforeEvent=await getBoothEventStorageCurrentQty(storeCode,item.barcode);
        const requestedOut=returned+unreturned;
        const actualOut=Math.min(Math.max(0,beforeEvent),requestedOut);
        const shortage=Math.max(0,requestedOut-actualOut);
      ...
      for(const operation of applied.reverse()){
        try{
          for(const log of operation.logs||[]){if(log?.id)await sb(`inventory_logs?id=eq.${encodeURIComponent(log.id)}`,{method:"DELETE",headers:{Prefer:"return=minimal"}});}
          if(operation.movement?.id)await sb(`event_storage_movements?id=eq.${encodeURIComponent(operation.movement.id)}`,{method:"DELETE",headers:{Prefer:"return=minimal"}});
          if(operation.storageMoved)await upsertBoothEventStorageStock(getBoothEventStoreCode({store_code:operation.item.store_code}),operation.item,operation.storageDelta);
          if(operation.baseMoved)await adjustBoothProductBaseStock(operation.item.barcode,-operation.baseDelta);
          await patchBoothEventItem(operation.item,getBoothCloseRestorePayload(operation.before));
        }catch(rollbackError){console.warn("[booth close rollback failed]",rollbackError);}
      }
      throw error;
  ...
  rollbackBoothCloseReflection=async function(){ return []; };
  ...
  finalizeBoothEventClose=async function(event,summary,staff){
    ...
    try{
      ...
      renderBoothEvents(boothEvents);
      renderBoothEventDetail(closedEvent);
      boothShowSuccess("\u30a4\u30d9\u30f3\u30c8\u7de0\u3081\u5b8c\u4e86","\u4eca\u56de\u30a4\u30d9\u30f3\u30c8\u5206\u306e\u5728\u5eab\u3092\u78ba\u5b9a\u3057\u3001\u30a4\u30d9\u30f3\u30c8\u3092\u7d42\u4e86\u3057\u307e\u3057\u305f\u3002");
  ...
  // Saving the report records return facts only. Inventory movement belongs to close.
  saveBoothReportReturnBatch=async function(){
    if(window.__aricoBoothReportReturnBatchSaving)return;
    const section=document.querySelector("[data-booth-report-return-section]");
    const event=getBoothCurrentEvent();
    const destination=normalizeBoothReportReturnDestination(section?.dataset.selectedDestination||"");
    if(!section||!event||!destination){boothShowError("戻り実績保存エラー","戻り先を選択してください。");return;}
    const entries=new Map();
    section.querySelectorAll("[data-booth-report-return-input]").forEach(input=>{
      const id=String(input.dataset.itemId||"");
    ...
    const changed=[...entries.values()].filter(row=>row.value!==String(row.saved));
    if(!changed.length){boothShowError("戻り実績保存エラー","変更した戻り実績がありません。");return;}
    if(changed.some(row=>!/^[0-9]+$/.test(row.value))){boothShowError("戻り実績保存エラー","戻り実数は0以上の整数で入力してください。");return;}
    const ok=typeof confirmAppAction==="function"?await confirmAppAction("戻り実績を保存",`戻り先：${getBoothReportReturnDestinationLabel(destination)}\n変更商品：${changed.length}件\n在庫移動はイベント締め時に実行します。`,{okText:"保存",cancelText:"キャンセル"}):true;
    if(!ok)return;
    window.__aricoBoothReportReturnBatchSaving=true;
    try{
      for(const entry of changed){
        const rows=await sb(`booth_event_items?select=*&event_id=eq.${encodeURIComponent(event.id)}&id=eq.${encodeURIComponent(entry.id)}&item_type=eq.normal&limit=1`);
        const item=Array.isArray(rows)&&rows[0]?rows[0]:null;
        if(!item)throw new Error("対象の戻り商品が見つかりません。");
        const nextReturned=Number(entry.value);
        if(nextReturned>calculateBoothReturnPlannedQty(item))throw new Error(`${item.product_name||item.barcode}: 戻り数が戻り予定数を超えています。`);
        const now=new Date().toISOString();
        await patchBoothEventItem(item,{returned_qty:nextReturned,difference_qty:calculateBoothDifference({...item,returned_qty:nextReturned}),return_process_type:destination,return_reflected:false,return_reflected_qty:0,return_reflected_at:null,return_reflected_by:null,shelf_return_qty:0,event_storage_qty:destination==="storage"?nextReturned:0,shelf_return_reflected:false,shelf_return_reflected_qty:0,shelf_return_reflected_at:null,shelf_return_reflected_by:null});
      }
      await refreshBoothEventRelatedViews(event.id); await loadBoothEventReport(event.id);
      boothShowSuccess("戻り実績を保存しました","在庫移動はイベント締め時に実行します。");
    }catch(error){boothShowError("戻り実績保存エラー",error.message||"戻り実績の保存に失敗しました。");}
    finally{window.__aricoBoothReportReturnBatchSaving=false;}
  };
...
(function(root){
  const originalCloseRender=renderBoothCloseConfirmPanel;
  root.renderBoothCloseConfirmPanel=function(event,summary){
    originalCloseRender(event,summary);
    const card=el("boothCloseCard")||document.querySelector(".booth-close-card");
    card?.querySelectorAll(".booth-close-warning,[data-booth-return-destination-change]").forEach(node=>node.remove());
    const table=card?.querySelector(".booth-close-table");
    if(table){
      table.querySelectorAll("tr").forEach(row=>row.children[6]?.remove());
      const head=table.querySelector("thead th:nth-child(6)");
      if(head)head.textContent="戻り実数";
    }
  };
  ...
  root.confirmBoothEventClose=confirmBoothEventClose=async function(event){
    ...
    try{
      ...
      const body=[`イベント：${latestEvent.name||"-"}`,`対象商品数：${rows.length}`,`戻り実数はすべて通常棚へ戻します。`,`未帰還：${rows.reduce((s,r)=>s+Number(r.unreturned_qty||0),0)}`,`共通イベント棚データ不足：${shortage}`,"不足があっても戻り実数を正として締めます。"].join("\n");
  ...
  root.saveBoothReportReturnBatch=saveBoothReportReturnBatch=async function(){
    if(window.__aricoBoothReportReturnBatchSaving)return;
    const section=document.querySelector("[data-booth-report-return-section]"),event=getBoothCurrentEvent();
    if(!section||!event){boothShowError("戻り実績保存エラー","イベントレポートを開いてください。");return;}
    const entries=[...section.querySelectorAll("[data-booth-report-return-input]")].map(input=>({id:String(input.dataset.itemId||""),value:String(input.value||"").trim(),saved:String(input.dataset.savedReturned||0)})).filter(row=>row.id);
    const changed=entries.filter(row=>row.value!==row.saved);
    if(!changed.length){boothShowError("戻り実績保存エラー","変更した戻り実績がありません。");return;}
    if(changed.some(row=>!/^[0-9]+$/.test(row.value))){boothShowError("戻り実績保存エラー","戻り実数は0以上の整数で入力してください。");return;}
