...
## v87 実運用向け棚卸し
- 商品コード読込後、実在庫数を手入力して「決定」で保存
- 実在庫 `0` の保存に対応
- 決定時に自動でチェック済み化
- 担当者プルダウンと直接入力に対応
- チェック済み件数、未チェック、除外商品の表示
- 不要商品の除外と復帰
- テスト入力や誤入力を戻せる「チェック解除」
- 全体CSV、差異のみCSVを追加
- 差異商品の原因確認パネルを追加
- 在庫管理シート履歴とスマレジ変動履歴を比較表示
- 旧商品別履歴の在庫チェック操作欄を非表示

既存環境では `supabase_schema_update_v87_stock_check_ops.sql` をSupabase SQL Editorで1回実行してください。
