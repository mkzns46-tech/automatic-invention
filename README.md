...
## v91 今回の差異一覧追加
- チェック完了後に、差異が出た商品だけを確認できる「今回の差異一覧」を追加
- 差異一覧から「原因確認」「商品履歴を見る」を直接操作可能
- 差異一覧は通常担当者も閲覧可能
- 差異のみCSV、チェック完了、チェック完了解除は責任者「田中」のみ操作可能
- スマレジ在庫は一覧画面に表示せず、内部計算とCSV出力でのみ使用

## v92 実在庫0・除外・差異一覧改善
- 実在庫 `0` の入力チェックを確実に保存できるように修正
- チェック不要商品を担当者選択後に「除外」できるように追加
- 除外した商品は「除外解除」で戻せるように追加
- 商品名と実在庫入力欄の距離を近づけるため、スマレジチェック表の列幅を調整
- 除外商品は差異一覧・差異のみCSVの対象外
